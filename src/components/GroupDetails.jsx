...
const Link = styled.a`
  ...
  justify-content: center;
`;
export default function GroupDetails({ list }) {
  let { id } = useParams();
  // let group = list[id]
  let { name, icon, details } = group;
  let { startDate, website, address, players } = details;
  return (
    <Container>
      {/* this is the : {id} */}
      <Title>{name}</Title>
      ...
      <Text>{startDate}</Text>
      <Link href={website}>go to site</Link>
      <Text>{address}</Text>
      <Title>Players</Title>
      <ul>
        {players.map((p) => {
          return (
            <li>
              ...
              <Text>shirt number: {p.number}</Text>
            </li>
          );
        })}
      </ul>
